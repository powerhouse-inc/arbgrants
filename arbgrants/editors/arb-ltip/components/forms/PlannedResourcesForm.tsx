...
import type { Status } from "document-models/arbitrum-ltip-grantee";
import { isActualsEmpty } from "document-models/arbitrum-ltip-grantee";
import ContractSelector from "../ContractSelector.js";
import type { IProps } from "../../editor.js";
import "react-datepicker/dist/react-datepicker.css";
import { editPhase } from "../../../../document-models/arbitrum-ltip-grantee/v1/gen/creators.js";
import validators from "../../../../document-models/arbitrum-ltip-grantee/v1/src/validators.js";
import { classNames, intHandler, toArray } from "../../util.js";
import PhaseTimespan from "../PhaseTimespan.js";
import useInitialScroll from "../../hooks/use-initial-scroll.js";
import useAllContracts from "../../hooks/use-all-contracts.js";
...
type PlannedResourcesFormProps = Pick<IProps, "dispatch"> & {
...
const PlannedResourcesForm = (props: PlannedResourcesFormProps) => {
  ...
  const startDate = useMemo(() => new Date(phase.startDate), [phase.startDate]);
  const endDate = useMemo(() => new Date(phase.endDate), [phase.endDate]);

  const isValid = useMemo(() => validators.isPlannedValid(planned), [planned]);
  ...
  const submit = useCallback(() => {
    ...
    dispatch(
      editPhase({
        ...
        status: phase.status === "Finalized" ? "Finalized" : "InProgress",
      }),
    );
  }, [
    dispatch,
  ...
  const description = useMemo(() => {
    ...
    if (now < startDate) {
      if (isValid) {
        return (
          <div>
            Once the two-week disbursement phase begins, you will be able to
            update the project actuals.
          </div>
        );
      }
      return (
        <div>
          Before the two-week disbursement phase begins, please enter the
          planned resources here. After the phase start date, you will be able
          to update the project actuals.
        </div>
      );
    } else {
      return (
        <div>
          The two-week disbursement phase has already begun. Please enter
          planned resources to continue.
        </div>
      );
    }
  }, [isValid, startDate]);
  const wrapperClasses = useCallback(
    (isValid: boolean) =>
      classNames(
        showErrors && !isValid ? "ring-2 ring-red-300" : "ring-1 ring-gray-300",
      ...
      <div>
        ...
        <div className={wrapperClasses(isDistributionMechanismsValid)}>
          ...
          <TagSelector
            ...
            onRemove={(value) =>
              setDistributionMechanismsLocal(
                distributionMechanismsLocal.filter((v) => v !== value),
              )
            }
          />
        </div>
