import type { Maybe, Phase } from "document-models/arbitrum-ltip-grantee";
...
export function correctPhases(phases: Maybe<Maybe<Phase>[]>): Phase[] {
  ...
  return (
    phases.slice(0, 7).map((p: Phase | null | undefined) => p as Phase) || []
  );
}
...
export function maybeToArray<T>(value: Maybe<Maybe<T>[]>): T[] {
  ...
  return value.filter(
    (v: T | null | undefined) => v !== null && v !== undefined,
  );
}
...
export const calculateStatus = (phases: Phase[]) => {
  ...
  for (const phase of phases) {
    ...
    const isStatsValid = !!phase.stats && validators.isStatsValid(phase.stats);
