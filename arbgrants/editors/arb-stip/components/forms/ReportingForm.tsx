...
import { correctPhases } from "../../../arb-ltip/util.js";

type ReportingFormProps = Pick<IProps, "dispatch"> & {
  phase: Phase;
  phaseIndex: number;
...
const ReportingForm = (props: ReportingFormProps) => {
  ...
  const arbRemaining = useMemo(() => {
    return (
      (state.grantSize || 0) - calculateArbReceived(correctPhases(state.phases))
    );
  ...
  const description = isValid ? (
    <div>
      ...
      <span className="font-bold">{dueDate}</span> to collect final statistics.
    </div>
  ) : (
    <div>
      While the two-week phase is ongoing, please track the project actuals
      here. This information should be submitted by
        ...
        <div className="relative rounded-md !rounded-t-none !rounded-b-none px-3 pb-1.5 pt-2.5 ring-1 ring-inset ring-gray-300 flex">
          <div
            className={classNames(
              showErrors && !isArbReceivedValid ? "ring-2 ring-red-300" : "",
          ...
          <div
            className={classNames(
              showErrors && !isArbUtilizedValid ? "ring-2 ring-red-300" : "",
              "flex-1 relative rounded-md !rounded-b-none !rounded-t-none px-3 pb-1.5 pt-2.5 ring-inset focus-within:z-10 focus-within:ring-2 focus-within:ring-purple-600",
            )}
          >
            <label className="block text-xs font-medium text-gray-900 flex items-baseline">
