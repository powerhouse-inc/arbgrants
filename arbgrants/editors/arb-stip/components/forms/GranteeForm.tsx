...
import { classNames, intHandler, maybeToArray } from "../../util.js";
...
// Vite's CJS interop on react-datepicker's UMD `browser` bundle resolves the default
// import to the module namespace object, so unwrap `.default` to get the class.
// eslint-disable-next-line @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
const DatePicker: any = (DatePickerBase as any).default ?? DatePickerBase;
...
type GranteeFormProps = ArbitrumStipGranteeState &
  Pick<IProps, "dispatch"> & { onClose: () => void };
...
const GranteeForm = (props: GranteeFormProps) => {
  ...
  // 12 weeks
  const [startDate, setStartDate] = useState(new Date(2024, 5, 17));
  ...
  const onSubmit = useCallback(() => {
    ...
    if (isInit) {
      ...
      if (isAdmin) {
        ...
        input.disbursementContractAddress = disbursementContractAddressLocal;
      }
      ...
      <div className="isolate -space-y-px rounded-md shadow-sm">
        ...
        {isInit && (
          <div className={wrapperClasses(isEditorAddressesValid)}>
            ...
            <input
              ...
              onChange={(e) => setEditorAddressesLocal(e.target.value)}
            />
          </div>
        )}
        {(isAdmin || isInit) && (
          <div className={wrapperClasses(isDisbursementContractAddressValid)}>
            <div className="flex items-baseline">
              <label className="text-xs font-medium text-gray-900">
        ...
        <div className={wrapperClasses(isFundingTypeValid)}>
          ...
          <TagSelector
            ...
            schema={fundingTypes}
            onAdd={(value) => {
              setFundingTypeLocal([...fundingTypeLocal, value as FundingType]);
            }}
            onRemove={(value) => {
              setFundingTypeLocal(fundingTypeLocal.filter((v) => v !== value));
            }}
          />
        </div>
        <div className="rounded-md !rounded-t-none !rounded-b-none px-3 pb-1.5 pt-2.5 ring-1 ring-inset ring-gray-300 focus-within:z-10 focus-within:ring-2 focus-within:ring-purple-600">
          ...
          <input
            ...
            onChange={(e) => setMetricsDashboardLinkLocal(e.target.value)}
          />
        </div>
